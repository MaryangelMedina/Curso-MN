...
with tab_mision:
    ...
    st.markdown(
        """
        El servicio de Medicina Nuclear debe evaluar el comportamiento de
        distintos radionúclidos al atravesar un material. Utilizá lo aprendido
        en las estaciones para interpretar los resultados.
        """
    )
    ...
    predominancias_caso = calcular_predominancia_foton(
        energia_caso,
        material_caso,
    )
    ...
    metrica_2.metric(
        ...
        interaccion_dominante,
    )

    metrica_3.metric(
        "Transmisión",
        f"{100 * transmision_caso:.2f} %",
    )
    ...
    st.markdown(
        f"""
        1. Explique por qué **{interaccion_dominante}** resulta conceptualmente
        predominante para **{fuente_caso}** en **{material_caso}**.

        2. Interprete físicamente el valor de transmisión obtenido para
        **{espesor_caso:.4g} cm** de material.

        3. Explique qué significa que el espesor represente
        **{numero_hvl_caso:.2f} semiespesores**.

        4. Proponga un colimador apropiado para la energía seleccionada.

        5. Indique al menos dos limitaciones del modelo de simulación utilizado.

        6. Diferencie una simulación didáctica de un cálculo real de blindaje
        o dosimetría.
        """
    )
